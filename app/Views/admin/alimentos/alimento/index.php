...
<body class="bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50 min-h-screen">
    ...
    <div class="flex pt-20">
        ...
        <aside id="sidebar"
            ...
            <div class="p-6 space-y-2">
                ...
                <a href="<?= base_url('admin/alimentos') ?>" class="sidebar-item active">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="max-w-6xl mx-auto px-4 lg:px-8 py-8">
                ...
                <div class="mb-8 animate-fade-in">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            <h1 class="text-4xl font-bold text-gray-900 mb-2">Gerenciar Alimentos</h1>
                            <p class="text-gray-500 flex items-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                                Total de alimentos: <strong id="total-alimentos" class="text-green-600">0</strong>
                            </p>
                        </div>

                        <a href="<?= base_url('admin/alimentos/criar') ?>"
                            class="inline-flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-green-500 to-green-600 text-white font-semibold rounded-xl shadow-md hover:from-green-600 hover:to-green-700 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-0.5">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-8 animate-fade-in">
                    <div class="flex flex-col sm:flex-row gap-4 items-end">
                        <div id="search-container" class="flex-1">
                            ...
                            <input type="text" id="search-input" placeholder="Digite o nome..." oninput="applyFilter()"
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden animate-fade-in">
                    <div class="overflow-x-auto">
                        <table class="w-full" id="alimentos-table">
                            <thead>
                                <tr class="bg-gray-50 border-b border-gray-200">
                                    <th class="px-8 py-4 text-left text-sm font-bold text-gray-700">Nome</th>
                                    <th class="px-8 py-4 text-left text-sm font-bold text-gray-700">Calorias</th>
                                    <th class="px-8 py-4 text-left text-sm font-bold text-gray-700">Proteinas</th>
                                    <th class="px-8 py-4 text-left text-sm font-bold text-gray-700">Carboidratos</th>
                                    <th class="px-8 py-4 text-center text-sm font-bold text-gray-700">Gorduras</th>
                ...
                <div class="px-8 py-6 bg-gray-50 border-t border-gray-200">
                    <p class="text-sm text-gray-600">Mostrando <strong id="showing-count">0</strong> de <strong
                            id="total-count">0</strong> alimentos</p>
                </div>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 animate-fadeIn">
            ...
            <div class="flex justify-end gap-3">
                ...
                <button onclick="confirmDelete()"
                    class="px-6 py-2.5 bg-red-600 text-white rounded-lg font-medium hover:bg-red-700 transition">
                    Deletar
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        function loadAlimentos() {
            const alimentosData = <?= json_encode($alimentos ?? []) ?>;
            allAlimentos = alimentosData;
            document.getElementById('total-alimentos').textContent = allAlimentos.length;
            document.getElementById('total-count').textContent = allAlimentos.length;
            renderTable(allAlimentos);
        }
        ...
        function renderTable(alimentos) {
                    ...
                    <tr class="table-row-hover border-b border-gray-100">
                        <td class="px-8 py-4">
                            <div class="flex items-center gap-3">
                                <span class="font-semibold text-gray-900">${alimento.nome}</span>
                            </div>
                        </td>
                        <td class="px-8 py-4">
                            <span class="text-gray-600 text-sm">${alimento.calorias} kcal</span>
                        </td>
                        <td class="px-8 py-4">
                            <span class="text-gray-600 text-sm">${alimento.proteinas}</span>
                        </td>
                        <td class="px-8 py-4">
                            <span class="text-gray-600 text-sm">${alimento.carboidratos}</span>
                        </td>
                        <td class="px-8 py-4">
                            <span class="text-gray-600 text-sm">${alimento.gorduras}</span>
                        </td>
                        <td class="px-8 py-4">
                            <div class="flex gap-3">
                                <a href="<?= base_url('admin/alimentos/editar') ?>/${alimento.id}" class="p-2 text-blue-500 hover:text-blue-700 hover:bg-blue-50 rounded-lg transition" title="Editar">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                    </svg>
                                </a>
                                <button onclick="openDeleteModal(${alimento.id}, '${alimento.nome}')" class="p-2 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-lg transition" title="Deletar">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            document.getElementById('showing-count').textContent = alimento.length;
        }
        ...
        function applyFilter() {
            ...
            const filtered = allAlimentos.filter(alimento => {
                if (filterType === 'nome') {
                    return alimento.nome.toLowerCase().includes(searchInput);
                }
                return true;
            });
        ...
        function openDeleteModal(receitaID, receitaName) {
            ...
            document.getElementById('delete-alimentos-name').textContent = receitaName;
        ...
        function confirmDelete() {
            if (deleteReceitaID) {
                window.location.href = `<?= base_url('admin/alimentos/deletar') ?>/${deleteReceitaID}`;
            }
        }

        // Carregar usuários ao iniciar a página
        document.addEventListener('DOMContentLoaded', loadAlimentos);
    </script>
